...
object bolichito {
	...
	method esBrillante() = objetoEnVidriera.material().esBrillante() and 
	...
	method tieneAlgoDeColor(color) = objetoEnVidriera.color() == color or 
		objetoEnMostrador.color() == color
	method puedeMejorar() = self.estaDesequilibrado() or self.esMonocromatico()
	method puedeOfrecerleAlgoA(persona) = persona.leGusta(objetoEnVidriera) or
